...
from vitsegh.config import TrainingConfig
...
def generate_binary_inference(
    ...
    if "0" in str(device):
        print("Computing inference on dataset...")
    ...
    with torch.no_grad():
        for inputs, labels in dataloader:
            ...
            output_tensor = prob_mask.cpu().detach().numpy()


            
            # Suppose output_tensor is your tensor with shape (8, 1, 512, 512)
            # Select the first image in the batch
            single_image = output_tensor[0]  # shape: (1, 512, 512)
            
            # Remove the channel dimension since it's 1
            single_image = single_image.squeeze(0)

#             # Display the image:
#             # - For grayscale/binary images, use cmap='gray'
#             # - For multi-channel images (e.g., RGB), ensure the shape is (H, W, 3)
#             if output_image.ndim == 2:
#                 plt.imshow(output_image, cmap='gray')
#             else:
#                 # If the image is in the format (channels, H, W), transpose it to (H, W, channels)
...
#                 plt.imshow(output_image)
            ...
            inference_result_path = path.join(output_path, "inference", "inference_output.tif")
            plt.savefig(inference_result_path, dpi=300, bbox_inches='tight')
